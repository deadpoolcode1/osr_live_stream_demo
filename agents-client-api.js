...
// Load the API key and other configurations
...
const startButton = document.getElementById('start-button'); // Ensure this is defined

// WebSocket setup - Added this section
...
socket.onmessage = function(event) {
  ...
  // Process the message as if it were entered via the UI
...
startButton.onclick = async () => {
  ...
  if (peerConnection?.signalingState === 'stable' || peerConnection?.iceConnectionState === 'connected') {
    ...
    if (playResponse.status === 200 && playResponseData.chatMode === 'TextOnly') {
      console.log('User is out of credit, API only returns text messages');
...
async function agentsAPIworkflow() {
  agentIdLabel.innerHTML = `<span style='color:orange'>Processing...</span>`;
  chatIdLabel.innerHTML = `<span style='color:orange'>Processing...</span>`;
